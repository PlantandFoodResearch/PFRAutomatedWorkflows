...
params.reads = "/input/genomic/plant/Actinidia/chinensis/ExperimentRequestor10616/*_R{1,2}.fastq.gz"
...
params.outdir = 'results47'   /* Defines an output directory where you will be able to see the output files/data of this pipeline */
...
process Trimmomatic {
	...
	executor "lsf"
        ...
        input:
        set pair_id, file(fqreads) from ReadChannel 
...
"""
java -jar /software/bioinformatics/Trimmomatic-0.36/trimmomatic.jar PE ${pair_id}_R1.fastq.gz  ${pair_id}_R2.fastq.gz -baseout ${pair_id}.trimmomatic.fq.gz ILLUMINACLIP:"/software/bioinformatics/Trimmomatic-0.36/adapters/TruSeq2-PE.fa":2:30:10 SLIDINGWINDOW:5:20 MINLEN:50
"""
...
while read LINE; do bash ${baseDir}/Scripts/testSQL.sh \$LINE ${species} ${pair_id}i >> buffer.sql; done < ${pair_id}_HTseq.C.counts 
