...
params.outdir = 'results39'   /* Defines an output directory where you will be able to see the output files/data of this pipeline */
params.outdir1= '010.QualityControlR' /* In this directory you will able to see all the Quality Control Report for all the steps */
params.outdir2= '020.TrimmingData' /* You could get back all the trimming data in this directory */
params.outdir3= '030.BamFiles' /* Get back all the bam files comes from the Alignment Step */
params.outdir4= '040.Statistics' /*Get back all the statistics about the bams files and  differential workflow expression*/
params.outdir5= '050.Index' /*Get back the directory of the index genome if it doesn't already exist*/
species='Potatoes'

/* If genome index directory already exist please inform the path to acceed it and put 1 in the indexC variable (0 else). Uncommented the next lines */
...
/*Channel.fromPath("/workspace/cfpcxs/PFRAutomatedWorkflows/RNAseq/results31/indexDir").ifEmpty{ error "Cannot find any reads matching: ${params.index}" }.set{genome_indexD}*/
...
process buildIndex {
    ...
    output:
    file("indexDirB") into genome_indexD  /* Ouput file is the index reference genome in the genome_index channel */
...
process alignment {
    ...
    input:
    file(index1) from genome_indexD
...
while read LINE; do bash /workspace/cfpcxs/PFRAutomatedWorkflows/RNAseq/testSQL3.sh \$LINE ${species} ${pair_id}; done < ${pair_id}_HTseq.counts
